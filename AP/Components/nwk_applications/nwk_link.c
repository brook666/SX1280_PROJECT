...
static uint8_t sDuplicateTid = 0;
static uint8_t sDuplicateLinkFlag = 0;
/******************************************************************************
 * LOCAL FUNCTIONS
 */
...
static void smpl_send_unlink_reply(mrfiPacket_t *frame)
{
  ...
  pCInfo = nwk_findPeer((addr_t *)MRFI_P_SRC_ADDR(frame), \
  ...
  pOutFrame = nwk_buildFrame(SMPL_PORT_LINK, msg, sizeof(msg), MAX_HOPS);
  if (pOutFrame)
  {
...
//#ifdef  END_ADD_EXEC
//    if ((lt != sLinkToken)&&(lt != sLinkToken_exe))
//    {
//      return SENT_NO_REPLY;
//    }
//#else
...
//#endif
  ...
  /*get IP addr*/
  nwk_joinList_searchIP((addr_t *)MRFI_P_SRC_ADDR(frame),&edIPAddr);
  
  /* is this a duplicate request? */
  remotePort = *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+L_RMT_PORT_OS);
  pCInfo=nwk_isLinkDuplicate(MRFI_P_SRC_ADDR(frame), remotePort);
  if (pCInfo)   
  {
    /* resend reply */
    ...
    msg[LR_ED_IPH_OS]    = edIPAddr.edAddr[1];
    msg[LR_ED_IPL_OS]    = edIPAddr.edAddr[0];
    msg[LR_AP_IPH_OS]    = edIPAddr.apAddr[1];
    msg[LR_AP_IPL_OS]    = edIPAddr.apAddr[0];
#if defined(SMPL_SECURE)
...
#endif
    pOutFrame = nwk_buildFrame(SMPL_PORT_LINK, \
                                msg, \
                                sizeof(msg), \
                                MAX_HOPS-(GET_FROM_FRAME(MRFI_P_PAYLOAD(frame),\
                                F_HOP_COUNT)));
    if (pOutFrame)
    {
      /* destination address is the source adddress of the received frame. */
...
//处理重复加入的节点，在主函数中进行相关的鲁棒性判断		
		nwk_setDuplicateLinkLid(pCInfo->thisLinkID);
		nwk_setDuplicateLinkFlag();
  ...
  if (pCInfo)
  {
    ...
    pOutFrame = nwk_buildFrame(SMPL_PORT_LINK, \
                               msg, \
                               sizeof(msg), \
                               MAX_HOPS-(GET_FROM_FRAME(MRFI_P_PAYLOAD(frame),F_HOP_COUNT)));
    if (pOutFrame)
    {
      /* destination address is the source adddress of the received frame. */
      memcpy(MRFI_P_DST_ADDR(&pOutFrame->mrfiPkt), MRFI_P_SRC_ADDR(frame), NET_ADDR_SIZE);
...
#ifdef SEGGER_DEBUG
  SEGGER_RTT_WriteString(0, RTT_CTRL_TEXT_BRIGHT_MAGENTA"HANDLE LINK!!!"RTT_CTRL_RESET"\n");
#endif
  }
...
/**************************************************************************************************
 * @fn          nwk_setDoubleLinkLid
 *
 * @brief       保存重新与网关连接的节点的lid
 *
 * @param       lid     - 连接标识符(LID)
 *
 * @return      none
...
/**************************************************************************************************
 * @fn          nwk_getDoubleLinkLid
 *
 * @brief       获取重新与网关连接的节点的lid
 *
 * @param       
 *
 * @return      连接标识符(LID)
...
{
  sDuplicateLinkFlag = 1;
}
/**************************************************************************************************
 * @fn          nwk_clearDoubleLinkLid
 *
 * @brief       保存重新与网关连接的节点的lid
 *
 * @param       void
...
{
  sDuplicateLinkFlag = 0;
}
/**************************************************************************************************
 * @fn          nwk_isExistDuplicateLinkED
 *
 * @brief       获取重新与网关连接的节点的lid
 *
 * @param       
