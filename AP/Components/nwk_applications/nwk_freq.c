...
uint8_t nwk_scanForChannels(freqEntry_t *channels)
{
  ...
  for (i=0; i<NWK_FREQ_TBL_SIZE; ++i)
  {
   ...
   /* uint8_t time_50_8_ms_count=0;
    TACCTL0 &= ~CCIFG;
    TAR = 0; // initial count  
    TACCR0 = 50000; // compare count. (delay in ticks) 

    // Start the timer in UP mode 
    TACTL |= MC_1;
    while( SMPL_SUCCESS != SMPL_Ioctl(IOCTL_OBJ_RAW_IO, IOCTL_ACT_READ, &ioctl_info.recv))
    {
      if(TACCTL0 & CCIFG)
      {
         if(time_50_8_ms_count==32)// 等待200ms
         {
           break;
         }
         else
...
#ifdef ACCESS_POINT
#ifndef ED_ACTIVE_FREQ_HOP_TEST
 ...
 */
#ifdef ACCESS_POINT
#define CC_REDUNDANCY      3   /* Change-channel redundancy count */
