...
** 版  本:  1.0
**
** 历  史:  2015-05-13  Leidi 在smpl_send_join_reply()中过滤源地址不在Join设备列表中的加入请求.
            2015-05-14  Leidi 在nwk_joinInit()中初始化Join设备列表.
***************************************************************************/
/******************************************************************************
 * INCLUDES
 */
...
#include "net_freq_agility.h"//zeng add 
/******************************************************************************
 * MACROS
 */
...
uint8_t join_token_wrong_count;   //zeng add 
#endif /* ACCESS_POINT */
 ...
 * @return   void
 */
void nwk_joinInit(uint8_t (*pf)(linkID_t,uint8_t))
{
  if (!sJoinToken) 
  {
...
static void smpl_send_join_reply(mrfiPacket_t *frame)
{
  ...
  /* see if join token is correct */
  {
    ...
    if (jt != sJoinToken)
    {
#ifdef COMP_JOIN_TOKEN  
#ifdef FREQUENCY_AGILITY  
#ifdef DEBUG
      put_string_uart1("join token is not right!!!\n");
#endif
      join_token_wrong_count++;
      if(join_token_wrong_count>=10)
      {
        changeChannel();  
        join_token_wrong_count=0;
      }
#endif   
#endif
      return;
  ...
  pOutFrame = nwk_buildFrame(SMPL_PORT_JOIN, msg, sizeof(msg), MAX_HOPS_FROM_AP);
  if (pOutFrame)
  {
    ...
    //中继器该为为0，AP不会监听该节点；节点该为不为0，AP监听该节点的连接请求
...
smplStatus_t nwk_join(void)
{
...
#else
  {
    ...
    put_string_uart1(" ED send join message:\n");
    for(uint8_t k=0; k<sizeof(msg);k++)
    {
      put_char_uart1(msg[k]); 
    }
    put_string_uart1("\n");
#endif
...
#ifdef DEBUG_1
    put_string_uart1(" ED receive reply of join message:\n");
    for(uint8_t k=0; k<sizeof(msg);k++)
    {
      put_char_uart1(msg[k]);  
    }
...
#ifdef DEBUG_1
    put_string_uart1("ED dont receive reply of join message:\n");
    put_string_uart1("\n");
#endif      
