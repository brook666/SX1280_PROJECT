...
static uint8_t  nwkFrameReceiveCount = 0;
																											
#if !defined(RX_POLLS)
static uint8_t  (*spCallback)(linkID_t,uint8_t) = NULL;
#endif
...
void nwk_frameInit(uint8_t (*pF)(linkID_t,uint8_t))
{
...
frameInfo_t *nwk_buildAckReqFrame(uint8_t port, uint8_t *msg, uint8_t len, uint8_t hops, volatile uint8_t *tid)
{
  ...
  fInfoPtr=nwk_buildFrame(port, msg, len, hops);
  if (!fInfoPtr)
  {
    return (frameInfo_t *)NULL;
  }
...
void MRFI_RxCompleteISR()
{
  ...
  /* 过滤源地址不在Join设备列表中的加入和连接请求，过滤掉一些广播帧的请求。
     不过滤遥控器发来的信息。现在主要是节点join和Link时会产生广播帧，过滤
     掉不在列表里面的节点。如果不对不在列表中的节点进行过滤，则当不在列表的
     节点发起Link时，AP会执行FHS_REPLAY，对信息进行转发。网络中有多个AP时，
     会对该信息不断的进行转发，知道跳数为0。
  */
  frameInfo_t  *fInfoPtrTmp = (frameInfo_t *)malloc(sizeof(frameInfo_t));
 
  MRFI_Receive(&fInfoPtrTmp->mrfiPkt);
  if((nwk_joinList_isFound((const addr_t *)MRFI_P_SRC_ADDR(&fInfoPtrTmp->mrfiPkt))) ||\
      !memcmp(nwk_getRemoterAddress(), MRFI_P_SRC_ADDR(&fInfoPtrTmp->mrfiPkt), NET_ADDR_SIZE))
  {
		nwkFrameReceiveCount++;
     /* room for more? */
    fInfoPtr=nwk_QfindSlot(INQ);
...
static void dispatchFrame(frameInfo_t *fiPtr)
{
  ...
  if (isForMe || ((port == SMPL_PORT_USER_BCAST) && !memcmp(nwk_getBCastAddress(), MRFI_P_DST_ADDR(&fiPtr->mrfiPkt), NET_ADDR_SIZE)))
  {
     ...
     * source address.
     */
    //nwk_isConnectionValid处理应答帧：如果收到的数据需要应答，则发送应答帧,并返回1；
    //如果收到的数据为应答帧，则判断应答是否正确，并返回0
    if (nwk_isConnectionValid(&fiPtr->mrfiPkt, &lid))
    {
      ...
      if (!isForMe)
      {
        ...
        //原协议里面AP收到广播帧后，转发该数据现改为不转发该数据。
//        nwk_replayFrame(fiPtr);
				//对遥控器发送的帧进行一次转发
				static addr_t  pre_addr ;
				addr_t cur_addr;
				
				//如果是集中器信息，直接pass掉
				if(fiPtr->mrfiPkt.frame[10]&0x20)
				{
					return;
				}
				if(!memcmp(nwk_getRemoterAddress(), MRFI_P_SRC_ADDR(&fiPtr->mrfiPkt), NET_ADDR_SIZE))
				{
					static uint8_t pre_remoter_cnt = 0;
					static uint8_t cur_remoter_cnt = 0;
				
					cur_remoter_cnt  = GET_FROM_FRAME(MRFI_P_PAYLOAD(&fiPtr->mrfiPkt), F_TRACTID_OS);
					
					memcpy(cur_addr.addr,&fiPtr->mrfiPkt.frame[24],NET_ADDR_SIZE);

#ifdef SEGGER_REMOTER_DEBUG
				SEGGER_RTT_WriteString(0, RTT_CTRL_TEXT_BRIGHT_MAGENTA"remoter packet:"RTT_CTRL_RESET"\n");
				SEGGER_RTT_put_multi_char(&fiPtr->mrfiPkt.frame[0], fiPtr->mrfiPkt.frame[0]+1);
#endif     					
					if((cur_remoter_cnt != pre_remoter_cnt) || memcmp(pre_addr.addr,cur_addr.addr,MAC_ADDR_SIZE))
					{
						nwk_replayRmoterFrame(fiPtr);
						memcpy(pre_addr.addr,cur_addr.addr,MAC_ADDR_SIZE);
						pre_remoter_cnt = cur_remoter_cnt;			
					}
					else 
					{
						nwk_replayRmoterFrame(fiPtr);
        ...
        (void)isForMe; //此句代替上一句
      }
      /* OK. Now I handle it... */
      fiPtr->fi_usage = FI_INUSE_UNTIL_DEL;
      frm_num=fiPtr->mrfiPkt.frame[18];
			
      if (spCallback && spCallback(lid,frm_num))
      {
        fiPtr->fi_usage = FI_AVAILABLE;
        return;
      }
...
frameInfo_t *nwk_getSandFFrame(mrfiPacket_t *frame, uint8_t osPort)
{
  ...
  fiPtr=nwk_QfindOldest(INQ, &rcv, USAGE_FWD);
  if (fiPtr)
  {
    return fiPtr;
  }
...
{
	mrfiPacket_t  remoterFrame ;

	memcpy(&remoterFrame,&pFrameInfo->mrfiPkt,sizeof(pFrameInfo->mrfiPkt));

	remoterFrame.frame[10] = 0x13; //设为
	
  MRFI_Transmit(&remoterFrame, MRFI_TX_TYPE_CCA);
