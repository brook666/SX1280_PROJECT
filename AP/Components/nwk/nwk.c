...
#error ERROR: No swsn device type defined
#endif
 ...
 * is declared 'const'.
 */
persistentContext_t sPersistInfo = {CONNTABLEINFO_STRUCTURE_VERSION};
 ...
 */
smplStatus_t nwk_nwkInit(uint8_t (*f)(linkID_t,uint8_t))
{
...
static void initializeConnection(connInfo_t *pCInfo)
{
  ...
  pCInfo->cur_state  =  CONNSTATE_CONNECTED;
...
void nwk_freeConnection(connInfo_t *pCInfo)
{
  ...
  pCInfo->cur_state = CONNSTATE_FREE;  //add 
#endif
}
...
/******************************************************************************
 * @fn          nwk_getLinkIdByAddress
 *
 * @brief       通过节点的MAC地址获得其LID.
 *
 * input parameters
 * @param   frame    - 指向节点的MAC地址结构体
 *
 * output parameters
 *
 * @return   返回指向节点的LID.
 */
linkID_t nwk_getLinkIdByAddress(addr_t *addr)
{
#if NUM_CONNECTIONS > 0
  uint8_t       i;
  connInfo_t   *ptr = sPersistInfo.connStruct;
...
addr_t *nwk_getAddressByLinkId(linkID_t linkid)
{
  ...
  connInfo_t *ed_info= nwk_getConnInfo(linkid); 
  
  return (addr_t *)ed_info->peerAddr;
#else
...
uint8_t nwk_saveJoinedDevice(mrfiPacket_t *frame)
{
  ...
  connInfo_t *ptr   = sPersistInfo.connStruct;
/****************************************************
当sPersistInfo结构体已满，清除掉那些只是加入而
没有进行Link的节点。
****************************************************/
//  if(sPersistInfo.nextJoinID == SYS_NUM_CONNECTIONS)
//  {
...
void nwk_getNumObjectFromMsg(void *src, void *dest, uint8_t objSize)
{
  ...
  {
    case 2:
      *((uint16_t *)dest) = swsn_ntohs(*((uint16_t *)dest));
      break;

    case 4:
      *((uint32_t *)dest) = swsn_ntohl(*((uint32_t *)dest));
      break;
  }
...
void nwk_putNumObjectIntoMsg(void *src, void *dest, uint8_t objSize)
{
  ...
  switch(objSize)
  {
    ...
    case 2:
      u16 = swsn_htons(*((uint16_t *)src));
      ptr = (uint8_t *)&u16;
      break;

    case 4:
      u32 = swsn_htonl(*((uint32_t *)src));
      ptr = (uint8_t *)&u32;
      break;
...
/***************************************************************************
 * @fn          nwk_resetUnlinkedED
 *     
 * @brief       删除sPersistInfo信息中只join而没有link的信息。
 *              (节点没有link进网关，则会产生一个join信息)
 *     
 * @data        2016年02月24日
 *     
 * @param       void
...
/***************************************************************************
 * @fn          nwk_resetlinkedED
 *     
 * @brief       删除sPersistInfo信息中LINK节点的信息。
 *     
 * @data        2016年02月24日
 *     
 * @param       void
 ...
 * @fn          nwk_getEDState
 *     
 * @brief       获取在网节点数、离线节点数及节点总数
 *     
 * @data        2016年02月24日
 *     
 * @param       pInNetED  - 指向在线节点的存储地址
 *              pOutNetED - 指向离线节点的存储地址
 *     
 * @return      节点总数
 ***************************************************************************
 */ 
...
{
  int8_t i = 0;
  for( i=NUM_CONNECTIONS-1;i>=(NUM_CONNECTIONS-sPersistInfo.net_ed_num);i--)
  {
    if(sPersistInfo.connStruct[i].connState==0x02)
    {
      (*pInNetED)++;  //innet ed
    }
    else
  ...
  return sPersistInfo.net_ed_num;
}
...
uint8_t nwk_getEDbasicInfo(linkID_t linkID,uint8_t * ed_info)
{
  ...
  if((rc && (CONNSTATE_CONNECTED == sPersistInfo.connStruct[idx].connState)))
  {
    ed_info[0] = sPersistInfo.connStruct[idx].cur_state;   //在网状态
    ed_info[1] = sPersistInfo.connStruct[idx].hops2target; //跳数
    
    /* ED的MAC地址以逆序汇报(如FE 08 03 01) */
    for(i=0; i<4; i++)
    {
      ed_info[2+i] = sPersistInfo.connStruct[idx].peerAddr[3-i];
    }
    
    ed_info[6] = sPersistInfo.connStruct[idx].sigInfo.rssi;//RSSI
    ed_info[7] = sPersistInfo.connStruct[idx].sigInfo.lqi;//LQI
