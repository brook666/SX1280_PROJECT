...
#define JOIN_TOKEN_PTR       0x001008   //Join Token(4字节)
#define LINK_TOKEN_PTR       0x00100C   //Link Token(4字节)
...
void WriteJoinTokenToFlash(const uint32_t joinToken)
{
  ...
  FLASH_WriteProgram(JOIN_TOKEN_PTR,(uint8_t*)&token,sizeof(uint32_t));
}
...
void WriteLinkTokenToFlash(const uint32_t linkToken)
{
  ...
  FLASH_WriteProgram(LINK_TOKEN_PTR,(uint8_t*)&token,sizeof(uint32_t));
}
