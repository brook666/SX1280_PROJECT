...
uint8_t broadInterval = BROAD_TIME;
uint8_t bcastRndDelay = 0; //以接收到的第一包有效数据的RSSI值作为随机发送定时广播的随机延时
/**************************************************************************************************
 * @fn          MRFI_Init
 *
...
#endif  						
         if (1)   //!MRFI_RxAddrIsFiltered(MRFI_P_DST_ADDR(&mrfiIncomingPacket))
         {
            /* ------------------------------------------------------------------
             *    Receive successful
             *   --------------------
