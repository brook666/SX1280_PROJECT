...
app_net_status_t get_join_link_token(const app_frame_t *in_frame)
{
  ...
  if(msg_id_0 == 0x00)
  {
    ...
    join.token.joinToken = htonl(join.token.joinToken);
    link.token.linkToken = htonl(link.token.linkToken);
...
app_net_status_t set_datetime(const app_frame_t *in_frame)
{
  ...
  if(msg_id_0 == 0x01)
  {
    ...
    daytime_s.hours   = in_frame->data[7];
    daytime_s.minutes = in_frame->data[8];
    daytime_s.seconds = in_frame->data[9];
    
    /* 获取当前日期时间 */
    BSP_getDateTime(&daytime_g);
    
    /* 只有当偏差过大时才更新日期时间 */
    if(abs(daytime_s.seconds - daytime_g.seconds) > 5 ||
       daytime_s.minutes != daytime_g.minutes ||
       daytime_s.hours != daytime_g.hours ||
       daytime_s.week != daytime_g.week)
    {
      /* 设置日期时间 */
      BSP_setDateTime(&daytime_s);
    }
