...
{  
  ...
  {  
    ...
    if(nwk_isExistDuplicateLinkED())
		{
			SWSN_checkDuplicateLinkEdIPList();
			nwk_clearDuplicateLinkFlag();
		}
    if(bro_cast_sem)
    {
      //BSP_TOGGLE_LED4();
    ...
    write_67frm_to_ed_tx_buffer();
    		
    /* 遍历并处理节点的发送缓冲区 */
    Iterates_tx_ed_buffer();     //发送缓冲区数据
    Iterates_rx_ed_buffer();     //处理接收缓冲区数据
    Iterates_tx_fail_ed_buffer(); //处理发送失败的数据
//  send_broad_time_sync();     
