...
smplStatus_t SMPL_Init(uint8_t (*f)(linkID_t))
{
  ...
  a[0]=rc;
#ifdef SEGGER_DEBUG
	SEGGER_RTT_put_multi_char((uint8_t *)a,1);
#endif 
  return rc;
}
...
smplStatus_t SMPL_SendOpt(linkID_t lid, uint8_t *msg, uint8_t len, txOpt_t options)
{
  ...
  {
#if defined(ACCESS_POINT)
    rc = nwk_sendFrame(pFrameInfo, MRFI_TX_TYPE_FORCED );//MRFI_TX_TYPE_FORCED  MRFI_TX_TYPE_CCA
#else 
 ...
 **************************************************************************************************
 */
int8_t SWSN_GetRemoterRssi(void)
{
 ...
 **************************************************************************************************
 */
uint8_t SWSN_DeleteConnToAP(void)
{
  /* 获取接收遥控器信息的RSSI */
  SMPL_Ioctl(IOCTL_OBJ_CONNOBJ, IOCTL_ACT_DELETE, &sLinkID1);//释放连接
  MRFI_SetApAddrFilter(FALSE);  // 关闭AP地址过滤,允许加入其它AP
