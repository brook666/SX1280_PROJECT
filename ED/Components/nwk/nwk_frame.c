...
/*
 * History:     2014-9-24   Leidi 添加有关过滤无效AP的代码.
 *              2014-9-25   Leidi 删除全局变量ApAddr,使用官方函数nwk_getAPAddress()获取AP地址.
 *              2014-10-8   Leidi 删除全局变量check_freq_pause_2s和相关无用的代码(zeng add).
 *              2014-12-09  Leidi 在dispatchFrame()中不过滤遥控器的MAC地址.
 */
...
static uint8_t apAddrFilterEnabled = 0;//AP地址过滤控制
static uint8_t hasAPFlag = 1;//1，表示存在AP;0，表示不存在AP 
...
static void ReTransimit(frameInfo_t *);
...
/****** Fill static values for the DEVICEINFO byte that will go in each frame ******/
  ...
  #if defined(ACCESS_POINT)
    sMyTxType = F_TX_DEVICE_AP;    //0x20
  #else
    sMyTxType = F_TX_DEVICE_RE;    //0x10
  #endif
#else
    sMyTxType = F_TX_DEVICE_ED;    //0x00
  #if defined(RX_POLLS)
    sMyRxType = F_RX_TYPE_POLLS;
  #endif
  #if defined(RX_USER)
    sMyRxType = F_RX_TYPE_USER_CTL;  //0x00
  #endif
#endif
/****** DONE fill static values for the DEVICEINFO byte that will go in each frame ******/

#if !defined(RX_POLLS)
  spCallback = pF;        //回调函数sCB地址的映射，网络层中调用 
#else
  (void) pF;
#endif

  sMyAddr = nwk_getMyAddress();   // THIS_DEVICE_ADDRESS
...
frameInfo_t *nwk_buildFrame(uint8_t port, uint8_t *msg, uint8_t len, uint8_t hops)
{
  ...
  MRFI_SET_PAYLOAD_LEN(&fInfoPtr->mrfiPkt, len+F_APP_PAYLOAD_OS);     //app_payload  3
...
static void ReTransimit(frameInfo_t *fiPtr)
{
  uint8_t  hops = GET_FROM_FRAME(MRFI_P_PAYLOAD(&fiPtr->mrfiPkt), F_HOP_COUNT);

  /* if hops are zero, drop frame. othewise send it. */
  if (hops--)
  {
    PUT_INTO_FRAME(MRFI_P_PAYLOAD(&fiPtr->mrfiPkt),F_HOP_COUNT,hops);
     ...
     */
    if (GET_FROM_FRAME(MRFI_P_PAYLOAD(&fiPtr->mrfiPkt), F_PORT_OS) <= SMPL_PORT_NWK_BCAST)
    {
      nwk_setSecureFrame(&fiPtr->mrfiPkt, MRFI_GET_PAYLOAD_LEN(&fiPtr->mrfiPkt)-F_APP_PAYLOAD_OS, 0);
    }
#endif
    MRFI_DelayMs(5);
    nwk_sendFrame(fiPtr, MRFI_TX_TYPE_CCA);
  }
  else
  {
...
static void dispatchFrame(frameInfo_t *fiPtr)
{
  ...
  fhStatus_t  rc;
  linkID_t    lid;
  addr_t      remoter_addr = {0xfe, 0xfe, 0xfe, 0xfe};  //遥控器的MAC地址   
#if defined(ACCESS_POINT)
  uint8_t loc;
#endif
#if !defined(END_DEVICE)
  ...
  /* 过滤无效AP:如果开启AP地址过滤, 且源地址不是当前连接的AP地址, 则不处理该帧. */
  if(apAddrFilterEnabled) //节点加入一个网络后才开启AP地址过滤 
  {
    //允许遥控器的MAC地址,和集中器地址。其它源地址信息均过滤
    if(memcmp(MRFI_P_SRC_ADDR(&fiPtr->mrfiPkt), nwk_getAPAddress(), NET_ADDR_SIZE) &&\
       memcmp(MRFI_P_SRC_ADDR(&fiPtr->mrfiPkt), &remoter_addr, NET_ADDR_SIZE))    
    {
      fiPtr->fi_usage = FI_AVAILABLE;
      return;
    }
  }
  else //没有加入任何网络，则过滤掉无效的节点地址。但允许遥控器地址
  {
       ...
       memcmp(MRFI_P_SRC_ADDR(&fiPtr->mrfiPkt), &remoter_addr, NET_ADDR_SIZE))  
    {
     ...
     if(F_TX_DEVICE_AP == GET_FROM_FRAME(MRFI_P_PAYLOAD(&fiPtr->mrfiPkt), F_TX_DEVICE))
     {
       nwk_setAPAlive(1);
     }
  }
  ...
  /*如果源地址是广播地址，则直接过滤*/
  if(!memcmp(nwk_getBCastAddress(), MRFI_P_SRC_ADDR(&fiPtr->mrfiPkt), NET_ADDR_SIZE))
  {
    fiPtr->fi_usage = FI_AVAILABLE;
    return;    
  }
  ...
  /* it's destined for a user app. */
    /* 如果关闭AP地址过滤，即节点没有加入网关，则只接收广播帧，且原地址必须是遥控器MAC地址 */
  if(!apAddrFilterEnabled &&\
     memcmp(MRFI_P_SRC_ADDR(&fiPtr->mrfiPkt), &remoter_addr, NET_ADDR_SIZE))    //允许遥控器的MAC地址
  {
    fiPtr->fi_usage = FI_AVAILABLE;
    return;
  }
...
/**************************************************************************************************
 * @fn          MRFI_SetApAddrFilter
 *
 * @brief       开启或关闭AP地址过滤.开启后将过滤非当前连接AP发来的帧.在SMPL_Init()成功后才允许开启.
 *              否则SMPL_Init()无法成功.
 *
 * @param       var TRUE  - 开启AP地址过滤
 *                  FALSE - 关闭AP地址过滤
 *
 * @return      none
 **************************************************************************************************
 */
void MRFI_SetApAddrFilter(uint8_t var)
{
  apAddrFilterEnabled = var;
}
 ...
 **************************************************************************************************
 */
uint8_t  MRFI_ApAddrIsFiltered(void)
{
  return apAddrFilterEnabled ; 
}
 ...
 * @fn          nwk_setAPAlive
 *
 * @brief       设置是否存在AP.设置依据：是否存在AP的广播帧 。
 *
 * @param       1  - 存在AP 
 *              0 - 不存在AP 
 *
 ...
 **************************************************************************************************
 */
void nwk_setAPAlive(uint8_t var)
{
  hasAPFlag = var;
}
 ...
 *                  
 *
 * @return      1，存在AP；0，不存在AP
 **************************************************************************************************
 */
uint8_t nwk_hasAP(void)
{
  return hasAPFlag ; 
}
