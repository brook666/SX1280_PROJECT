 ...
 * LOCAL VARIABLES
 */
static addr_t         sMyROMAddress = THIS_DEVICE_ADDRESS;
...
{
  uint8_t *sPMacAddr = bsp_getMacAddr();
  memset(&sAPAddress, 0x00, sizeof(addr_t));
  memset(&sMyIPAddress, 0x00, sizeof(ip_addr_t));
  
  memcpy(&sMyROMAddress, sPMacAddr, MAC_ADDR_SIZE);
  /* set MAC address¡£populate RAM address from ROM default if it hasn't laready been set
  ...
  sMyIPAddress.edAddr[0]= sMyRAMAddress.addr[0];
  sMyIPAddress.edAddr[1]= sMyRAMAddress.addr[1];
