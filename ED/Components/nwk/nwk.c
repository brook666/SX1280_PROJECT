...
volatile net_frm_back_t net_ack_back = {0};
/******************************************************************************
 * GLOBAL VARIABLES
 */
...
smplStatus_t nwk_nwkInit(uint8_t (*f)(linkID_t))
{
  ...
  sPersistInfo.numConnections   = SYS_NUM_CONNECTIONS;    //0x03
  sPersistInfo.curNextLinkPort  = SMPL_PORT_USER_MAX;     //0x3d
  sPersistInfo.curMaxReplyPort  = PORT_BASE_NUMBER;       //0x20
  sPersistInfo.nextLinkID       = 1;

  /* initialize globals */
  nwk_globalsInit();   //初始化RAM address，sMyRAMAddress = THIS_DEVICE_ADDRESS；
  ...
  if (CONNSTATE_FREE == sPersistInfo.connStruct[NUM_CONNECTIONS].connState)   //初始化广播地址及其相关参数
  {
    ...
    sPersistInfo.connStruct[NUM_CONNECTIONS].portRx      = SMPL_PORT_USER_BCAST;   //0x3f
          ...
          *lid = ptr->thisLinkID;
          ...
          if (!(SMPL_PORT_USER_BCAST == port))
          {
            ...
            else if (GET_FROM_FRAME(MRFI_P_PAYLOAD(frame), F_ACK_RPLY))
            {
              ...
              {
                ptr->ackTID = 0;
					  		net_ack_back.isBack = 1;
                net_ack_back.lid = *lid;
              }
