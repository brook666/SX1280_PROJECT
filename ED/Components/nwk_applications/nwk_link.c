/**************************************************************************************************
  ...
  History:     2014-9-25   Leidi 在nwk_link()中, 如果FLASH中的AP地址有效, 则不使用广播地址.
                                 删除被注释屏蔽的陈旧代码.
               2015-07-29  Leidi 移除只加入特定网关的代码.
                                 删除有关pCInfo->cur_state的代码.
               2015-09-02  Zengjia 在nwk_processLink()中加入收到link回复帧后，设置节点IP代码
**************************************************************************************************/
/******************************************************************************
 * INCLUDES
 */
...
#ifdef SEGGER_DEBUG
SEGGER_RTT_WriteString(0, RTT_CTRL_TEXT_BRIGHT_MAGENTA"4"RTT_CTRL_RESET"\n");
#endif
...
fhStatus_t nwk_processLink(mrfiPacket_t *frame)
{
  ...
  if (SMPL_MY_REPLY == (replyType=nwk_isValidReply(frame, sTid, LB_REQ_OS, LB_TID_OS)))
  {
    ...
    rc = FHS_KEEP;
    //zeng add
    if(frame->frame[0]== 0x13)
    {
      myIPAddr.edAddr[1] = *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+LR_ED_IPH_OS);
      myIPAddr.edAddr[0] = *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+LR_ED_IPL_OS);
      myIPAddr.apAddr[1] = *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+LR_AP_IPH_OS);
      myIPAddr.apAddr[0] = *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+LR_AP_IPL_OS);
      nwk_setMyIPAddress(&myIPAddr);
    }
  }
